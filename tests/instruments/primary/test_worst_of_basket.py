...
class TestWorstOfBasketStock:
    ...
    def test_worst_of_logic_gpu(self):
        self.test_worst_of_logic(device=select_most_accurate_gpu_device())
    ...
    def test_correlation_effect(self, device: str = "cpu"):
        ...
        # With high correlation, worst-of should have higher volatility due to reduced diversification
        assert corr_std > uncorr_std

    @pytest.mark.gpu
    def test_correlation_effect_gpu(self):
        self.test_correlation_effect(device=select_most_accurate_gpu_device())
    ...
    def test_volatility_property_gpu(self):
        self.test_volatility_property(device=select_most_accurate_gpu_device())
    ...
    def test_volatility_property_before_simulation(self):
        ...
        with pytest.raises(AttributeError, match="WorstOfBasketStock has no buffer named volatility"):
            _ = s.volatility
    ...
    def test_custom_init_state(self, device: str = "cpu"):
        ...
        assert_close(s.basket[:, 0, 0], torch.full((3,), 100.0, device=device))
        assert_close(s.basket[:, 0, 1], torch.full((3,), 110.0, device=device))

    @pytest.mark.gpu
    def test_custom_init_state_gpu(self):
        self.test_custom_init_state(device=select_most_accurate_gpu_device())
    ...
    def test_different_parameters_gpu(self):
        self.test_different_parameters(device=select_most_accurate_gpu_device())
    ...
    def test_tensor_parameters_gpu(self):
        self.test_tensor_parameters(device=select_most_accurate_gpu_device())
    ...
    def test_to_device(self):
        ...
        # to(device)
        gpu_device = f"{select_most_accurate_gpu_device()}:0"
        s = WorstOfBasketStock(n_assets=2).to(device=gpu_device)
        assert s.device == torch.device(gpu_device)

        # Check parameter tensors are on correct device
        assert s.sigma.device == torch.device(gpu_device)
        assert s.mu.device == torch.device(gpu_device)
    ...
    def test_reproducibility_gpu(self):
        self.test_reproducibility(device=select_most_accurate_gpu_device())
